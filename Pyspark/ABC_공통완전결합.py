...
A_B = spark.read.csv('ABC_공통완전결합.csv', header=True, inferSchema=True)
...
# A_B를 기준으로 A와 조인
result_A = A_B.join(A, A_B.A_id  == A.id, how="full")
...
# A_B를 기준으로 B와 조인
result_B = A_B.join(B, A_B.B_id == B.id, how="full")
...
# A_B를 기준으로 C와 조인
result_C = A_B.join(C, A_B.C_id == C.id, how="full")
